...
# ----- NUEVO: mapping de etiquetas para hover -----
def map_strength_label(x):
    if pd.isna(x):
        return "N/A"
    # 3→high, 2→moderate, 1.5→low-moderate, 1→low, 0.5→very low
    if x >= 2.6:
        return "high"
    elif x >= 2.4:
        return "moderate"
...
def map_direction_label(x):
    if pd.isna(x):
        return "N/A"
    # 1→positive; ≥0.8→mostly positive; ≈0→inconclusive/mixed; ≤-0.8→negative; resto→mostly negative
    if x >= 1.0:
        return "positive"
    elif x >= 0.8:
...
fig.update_layout(
    ...
    coloraxis_colorbar=dict(
        ...
        tickmode="array",
        tickvals=[0.5, 1.0, 1.5, 2.0, 3.0],
        ticktext=["very low", "low", "low-moderate", "moderate", "high"]
    ),
)
...
app.layout = html.Div([
    ...
    html.P("The Strength of Evidence is defined on a scale from 0.5 (very low) to 3 (high), represented by a color gradient ranging from red (weaker evidence) to green (stronger evidence). The Direction of Effect is classified as Positive (+1), Inconclusive/Mixed (0), or Negative (–1). Average values can be viewed by clicking on each bubble."),
