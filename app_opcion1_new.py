...
strength_colors = {
    ...
    "Low":            "#f46d43",  # medium red
    "Low-moderate":   "#fdae61",  # orange
    "Moderate":       "#d9f0d3",  # light green
    "Moderate-high":  "#74c476",  # medium green
    "High":           "#006d2c",  # dark green
    "N/A":            "#C0C0C0"   # gray for missing values
}
...
# Elegimos 3 valores "bonitos" (min, mediana, max)
min_n_leg = int(max(1, agg["n"].min()))
med_n_leg = 5
max_n_leg = int(max(1, agg["n"].max()))

def nice(n):
    if n <= 5:
        return max(1, n)
    p = 10 ** max(0, len(str(n)) - 1)   # 1, 10, 100, ...
    return max(1, int(round(n / p) * p))

legend_ns = sorted({nice(min_n_leg), nice(med_n_leg), nice(max_n_leg)})
while len(legend_ns) < 3:
    legend_ns.add(max(1, max(legend_ns) - 1))
legend_ns = sorted(list(legend_ns))[:3]

# Posición y caja (paper coords)
box_x0, box_y0 = 1.02, 0.8      # esquina sup-izq del recuadro
box_w, box_h  = 0.26, 0.15       # ancho y alto del recuadro
...
for i, n_val in enumerate(legend_ns):
    d_px = n_to_px_diameter(n_val, scale=1.25)
    y_item = y_start - i * gap

    # círculo
    fig.add_annotation(
        xref="paper", yref="paper",
        x=box_x0 + 0.04, y=y_item,
...
fig.update_layout(
    height=fig_height,
    width=fig_width,
    margin=dict(l=520, r=300, t=90, b=320),  # margen derecho mayor para alojar la caja
    xaxis=dict(
        title="",
